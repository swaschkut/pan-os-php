...
class AddressGroup
{
     ...
     *
     */
    function __construct($name, $owner, $fromTemplateXml = FALSE, $type = 'static')
    {
        $this->owner = $owner;
        ...
        if( $fromTemplateXml )
        {
            ...
            {
                if( $type == "dynamic" )
                    $doc->loadXML(self::$templatexml_v6_dynamic, XML_PARSE_BIG_LINES);
                else
    ...
    public function addFilter($newFilter, $rewriteXml = TRUE)
    {
        if( !$this->isDynamic )
            derr('cannot be used on NONE Dynamic Address Groups');
        ...
        */
        ...
        {
            if( $this->owner->owner->version >= 60 )
                DH::createElement($this->filtersRoot, 'filter', "'".$newFilter."'");
        }
    ...
    static protected $templatexml_v6_dynamic = '<entry name="**temporarynamechangeme**"><dynamic></dynamic></entry>';
