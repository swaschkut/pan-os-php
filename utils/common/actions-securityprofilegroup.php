...
SecurityProfileGroupCallContext::$supportedActions[] = array(
    ...
    'GlobalFinishFunction' => function (SecurityProfileGroupCallContext $context) {
        ...
        if( $adoption )
            $headers .= '<th>adoption group</th>';


        if( $context->deviceType !== "scm" )
        {
            //only add if no Snippet/Container/DeviceOnPrem/DeviceCloud
        ...
        if( $context->deviceType === "scm" )
        {
            $headers .= '<th>WildFire and Virus</th>';
            if( $bestPractice )
                $headers .= '<th>BP WF/AV</th>';
            if( $visibility )
                $headers .= '<th>visibility</th>';
            if( $adoption )
                $headers .= '<th>WF/AV adoption</th>';

            //Todo: add DNS Security
            $headers .= '<th>DNS Security</th>';
            if( $bestPractice )
    ...
    'args' => array('filename' => array('type' => 'string', 'default' => '*nodefault*'),
        'additionalFields' =>
            array('type' => 'pipeSeparatedList',
                'subtype' => 'string',
                'default' => '*NONE*',
                'choices' => array('WhereUsed', 'UsedInLocation', 'TotalUse', 'BestPractice', 'Visibility', 'Adoption'),
                'help' =>
