...
// Helper function: converts EUR/MWh to rounded whole Euro Cents per kWh
function getPriceForTime($masterData, $targetTimestamp) {
    // 1. Exact 15-minute match
    $rounded15m = floor($targetTimestamp / 900) * 900;
    if (isset($masterData[$rounded15m])) {
        return (int)round($masterData[$rounded15m] / 10, 0);
    }

    // 2. Fallback to start of the hour (for 60-minute interval sources)
...
for ($curr1 = $startTimestamp1; $curr1 <= $endTimestamp1; $curr1 += 900) {
    ...
    $finalSeries1[] = getPriceForTime($masterData, $curr1);
    $finalSeries2[] = getPriceForTime($masterData, $curr2);
    $finalSeries3[] = getPriceForTime($masterData, $curr3);
}
...
<head>
    ...
    <title>3-Year Electricity Price Comparison Overlay</title>
    ...
    <style>
        ...
        .comparison-summary { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; margin-bottom: 25px; }
...
<div class="container">
    <h2>3-Year Electricity Price Comparison Overlay</h2>
    ...
    <div class="comparison-summary">
        <div class="period-box p1">
            <div class="period-title" style="color: #007bff;">Period 1 (<?php echo $dt1Start->format('Y-m-d'); ?>)</div>
            <div class="stats-row">
...
<script>
    ...
    const priceChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Selected Period (<?php echo $year1; ?>)',
                    data: series1,
                    borderColor: '#007bff',
