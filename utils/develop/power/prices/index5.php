...
// Base tax/fees input (Default: 18.665 cents before 19% VAT)
$taxInput = isset($_GET['tax']) ? str_replace(',', '.', $_GET['tax']) : '18.665';
$baseTax  = (float)$taxInput;

// Add 19% VAT to the base tax input
$taxAddOn = $baseTax * 1.19;
...
if (isset($_GET['shift'])) {
    ...
    // Redirect dynamically to current script preserving timezone, tax, and updated dates
    $newStart = date('Y-m-d\TH:i', $newStartTimestamp);
    $newEnd   = date('Y-m-d\TH:i', $newEndTimestamp);
    header("Location: " . $_SERVER['SCRIPT_NAME'] . "?start=" . urlencode($newStart) . "&end=" . urlencode($newEnd) . "&tz=" . $tzOffset . "&tax=" . urlencode($taxInput));
    exit;
}
...
// Helper function: standard 15-minute resolution with tax addition (includes 19% VAT)
function getPriceForTime15m($masterData, $targetTimestamp, $taxAddOn) {
    $rounded15m = floor($targetTimestamp / 900) * 900;
    if (isset($masterData[$rounded15m])) {
        $baseCents = $masterData[$rounded15m] / 10;
        return (int)round($baseCents + $taxAddOn, 0);
    }

    $hourly = floor($targetTimestamp / 3600) * 3600;
    if (isset($masterData[$hourly])) {
        $baseCents = $masterData[$hourly] / 10;
        return (int)round($baseCents + $taxAddOn, 0);
    }
...
// Helper function: 60-minute value calculated as average of 4 x 15-minute slots with tax addition
function getPriceForTime60m($masterData, $targetTimestamp, $taxAddOn) {
    $hourly = floor($targetTimestamp / 3600) * 3600;
    ...
    if (isset($masterData[$t0], $masterData[$t15], $masterData[$t30], $masterData[$t45])) {
        $avgRawCents = (($masterData[$t0] + $masterData[$t15] + $masterData[$t30] + $masterData[$t45]) / 4) / 10;
        return (int)round($avgRawCents + $taxAddOn, 0);
    }
...
for ($curr1 = $startTimestamp1; $curr1 <= $endTimestamp1; $curr1 += 900) {
    ...
    // Fetch rounded data values in Euro cents with tax added
    $finalSeries1[] = getPriceForTime15m($masterData, $curr1, $taxAddOn);
    $finalSeries2[] = getPriceForTime15m($masterData, $curr2, $taxAddOn);
    $finalSeries3[] = getPriceForTime15m($masterData, $curr3, $taxAddOn);
    $finalSeries4[] = getPriceForTime60m($masterData, $curr1, $taxAddOn);
}
...
<div class="container">
    ...
    <div class="controls-wrapper">
        <form method="GET" action="<?php echo $currentScript; ?>">
            ...
            <div class="form-group">
                <label for="tax">Base Tax/Fees (ct/kWh, excl. 19% VAT):</label>
                <input type="text" id="tax" name="tax" style="width: 100px;" value="<?php echo htmlspecialchars($taxInput); ?>">
            </div>
            <div class="form-group">
        ...
        <div class="shift-controls">
            ...
            <a href="<?php echo $currentScript; ?>?start=<?php echo urlencode($startDate); ?>&end=<?php echo urlencode($endDate); ?>&tz=<?php echo $tzOffset; ?>&tax=<?php echo urlencode($taxInput); ?>&shift=-1month" class="btn-shift">« -1 Month</a>
            <a href="<?php echo $currentScript; ?>?start=<?php echo urlencode($startDate); ?>&end=<?php echo urlencode($endDate); ?>&tz=<?php echo $tzOffset; ?>&tax=<?php echo urlencode($taxInput); ?>&shift=-7days" class="btn-shift">‹ -7 Days</a>
            <a href="<?php echo $currentScript; ?>?start=<?php echo urlencode($startDate); ?>&end=<?php echo urlencode($endDate); ?>&tz=<?php echo $tzOffset; ?>&tax=<?php echo urlencode($taxInput); ?>&shift=%2B7days" class="btn-shift">+7 Days ›</a>
            <a href="<?php echo $currentScript; ?>?start=<?php echo urlencode($startDate); ?>&end=<?php echo urlencode($endDate); ?>&tz=<?php echo $tzOffset; ?>&tax=<?php echo urlencode($taxInput); ?>&shift=%2B1month" class="btn-shift">+1 Month »</a>
        </div>
    </div>
