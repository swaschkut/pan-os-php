...
fig.update_layout(
    ...
    polar=dict(
        ...
        radialaxis=dict(
            ...
            ticks='',
			showgrid=True,       # <- notwendig, sonst wird gridcolor ignoriert!
            gridcolor='rgba(0,0,0,0)',  # <- komplett transparent machen
        ),
        angularaxis=dict(
            tickmode='array',
            tickvals=theta,
            ...
            direction='clockwise',
            rotation=78.1,
			showline=False,      # <- entfernt ggf. die 0° Linie
            gridcolor='rgba(0,0,0,0)',   # <- entfernt die „Tortenstücke“-Linien
            linewidth=0,
            linecolor='rgba(0,0,0,0)',
            layer="below traces"  # 👈 wichtig!
        )
    ),
    showlegend=False,
