    ...
    const radialLineColor = isLightMode ? '#cccccc' : '#666666';
    ...
    // Grid label colors
    const gridLabelColors_old = {
    25: { stroke: '#E8121C', fill: '#8B1820' },
    50: { stroke: '#F06A0A', fill: '#894617' },
    75: { stroke: '#FBB441', fill: '#6B552F' },
    100: { stroke: '#25B197', fill: '#1F635B' }
};
    ...
    .range([0, 2 * Math.PI])
    .paddingInner(0.005); // Reduced from 0.02 to make separators thinner
    ...
    .padAngle(0.005); // Reduced from 0.01 to make separators thinner
    ...
    .style('stroke-width', 1) // Reduced from 2 to make circles thinner
    ...
    const startAngle = angleScale(firstItem.name);
    const endAngle = angleScale(lastItem.name) + angleScale.bandwidth();
    ...
    const pathRadius = labelRadius + 15;
    let pathData;
    ...
    .style('font-weight', '300') // Reduced from 400 to make less bold
    .text(groupData.name);
});
