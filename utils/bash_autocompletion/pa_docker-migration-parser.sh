...
__pa_docker-migration-parser_scripts()
{
...
if [ -n "$ZSH_VERSION" ]; then
  ...
  if (( "${BASH_VERSINFO[0]}" > 4)); then
     complete -o default -F __pa_docker-migration-parser_scripts pa_docker-migration-parser
  else
     #Todo:
     ...
     complete -o nospace -F __pa_docker-migration-parser_scripts pa_docker-migration-parser
